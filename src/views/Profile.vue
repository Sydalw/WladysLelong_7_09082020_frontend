                    ...
                    <div class="min-w-screen flex items-center justify-center px-5 pb-5">
                        <div class="w-full max-w-4xl mx-auto bg-white border border-gray-200 rounded-xl overflow-hidden text-gray-800">
                            ...
                            <div class="rounded-full w-44 h-44 mx-auto md:mx-7 border-4 border-white bg-white -mt-28 md:-mt-32 z-10 relative overflow-hidden">
                                <img :src="getImgUrl($store.state.infosConnectedProfile.pictureURL)" :alt="$store.state.infosConnectedProfile.username" class="w-full h-full">
                            </div>
                            <div class="w-full px-8 pt-3 pb-8">
                                <div class="-mx-3 md:flex">
                                    <div class="px-3 w-full md:w-2/3 text-center md:text-left mb-5">
                                        <div class="inline-flex">
                                            <h1 class="font-semibold text-2xl">{{$store.state.infosConnectedProfile.username}}</h1>
                                            <input v-model="$store.state.champsInscription.pseudo" type="text" class="hidden w-full -ml-10 pl-10 pr-3 py-2 rounded-lg outline-none focus:border-blue-900" :placeholder="$store.state.infosConnectedProfile.username">
                                            <a href="#" class="pt-2 ">
                                        ...
                                        <h3 class="text-xl">{{$store.state.infosConnectedProfile.prenom}} {{$store.state.infosConnectedProfile.nom}}</h3>
                                        <h4 class="text-md text-gray-500">Membre depuis le {{$store.state.infosConnectedProfile.createdAt.split('T')[0]}}</h4>
                                    </div>
                                        ...
                                        <div class="text-center md:text-left">
                                            <div class="inline-block md:block mb-2 mx-3">
                                                <a href="#" class="font-semibold text-blue-500 hover:text-blue-600 group">
                                                    <span class="inline-flex items-center justify-center w-8 h-8 text-lg rounded-full bg-blue-500 group-hover:bg-blue-600 text-white mr-2"><i class="mdi mdi-email-outline text-md"></i></span> <span class="text-sm group-hover:underline">{{$store.state.infosConnectedProfile.email}}</span>
                                                </a>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div class="w-full text-center md:text-justify">
                                    <p class="text-sm leading-tight">{{$store.state.infosConnectedProfile.bio}}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="min-w-screen flex flex-col items-center justify-center px-5 py-5">
                        <topic v-for="infoTopic in infosTopic" :key="infoTopic.id" :id="infoTopic.id" :username="infoTopic.username" :title="infoTopic.title" :content="infoTopic.content" :createdAt="infoTopic.createdAt" :likes="infoTopic.Likes" :dislikes="infoTopic.Dislikes" :comments="infoTopic.CommentsNb" :pictureURL="infoTopic.pictureURL"></topic>
                    </div>
                </div>
            </template>
        </layout>
