<template>
    <form>
        ...
        <champForm :largeur="infosChamps[1].largeur" :for_id="infosChamps[1].for_id" :nom="infosChamps[1].nom" :type="infosChamps[1].type" :placeholder="infosChamps[1].placeholder">
          <template v-slot:input>
            <input v-model="$store.state.champsConnexion.password" v-validate="'required|password'" :class="{'input': true, 'border-red-500': errors.has(infosChamps[1].nom) }" :name="infosChamps[1].nom" :type="infosChamps[1].type" :id="infosChamps[1].for_id" class="w-full -ml-10 pl-10 pr-3 py-2 rounded-lg border-2 border-gray-200 outline-none focus:border-blue-900" :placeholder="infosChamps[1].placeholder">
            <div>
